...
wget --no-check-certificate -O "/tmp/n2n.zip" "https://github.do/https://raw.githubusercontent.com/lucktu/n2n/master/$(echo ${KERNEL} | sed -e 's/\b\(.\)/\u\1/g')/${FILE_NAME}"
unzip -o -d /tmp/n2n/ "/tmp/n2n.zip"

cd /tmp/n2n/

TAR_FILE_NAME=$(ls |grep '_static_'|grep  '_zstd_')
